...
> ⚠️ **替换同名文件时要注意缓存**：`assets/` 下的文件 CDN 缓存是 **4 小时**，
> 直接用同名新文件覆盖后，访客仍会拿到旧版本。两种解决办法：
> 1. 在 `content.json` 的 url 后面加版本参数，例如 `xxx.pdf?v=2`（换新文件就往上加）
> 2. 去 Cloudflare 后台「缓存 → 清除缓存 → 清除全部」
