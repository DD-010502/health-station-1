...
**只有 3 条是真正的成品内容**，其余都还是空位：

| 模块 | PDF | 视频 |
|---|---|---|
| 营养饮食 `diet` | ⚠️ 有卡片但文件不存在 | ⚠️ 有卡片但是示例链接 |
| 积极运动 `exercise` | 空 | 空 |
| 良好睡眠 `sleep` | 空 | 空 |
| 合理视屏 `screen` | 空 | 空 |
| 禁烟禁酒 `habits` | ✅ 应对话术 | 空 |
| 心理健康 `mental` | 空 | ✅ 2 个（公众号 + B 站） |
| 环环相扣 `loop` | 空 | 空 |
| 计划与行动 `action` | 空 | 空 |
...
1. **营养饮食的"青少年膳食指南手册"是坏链接** —— 卡片能显示，但点击会 404，因为文件从没上传过。
2. **营养饮食的视频"营养早餐怎么搭？"是假数据** —— 作者写的是"示例作者"，链接是微信示例参数，不是真文章。
3. **公众号二维码没接上** —— `content.json` 里预留了二维码位置，但图片文件不存在，而且目前没有任何页面去读取它。

修法都很简单：问题 1、2 要么补上真文件/真链接，要么把 `title` 和 `url` 清空（卡片就不显示了）；问题 3 需要先拿到二维码图片。
